...
> For type reference, click <a href="/types#actionhandler" style={{textDecoration: "underline", color:"lightblue"}}>here</a>
...
```ts // app/actions/index.ts "use server";
import { blogsRouter } from "@/lib/action.router.ts";
  ...
  <Tabs.Tab>
    ...
    ```ts
      ...
      .run(async (request, repsonse) => {
        // to access the final context
        console.log(request.context);
        // error message will be pulled out from your router config based on the error code you specify
        // ✨ inference magic
